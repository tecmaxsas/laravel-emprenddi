    ...
    <div style="background:#eef2ff; border:1px solid #6366f1; border-radius:10px; padding:14px 16px; margin-bottom:14px;">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
            <div>
                ...
                <div style="font-size:12.5px; color:#4c1d95; margin-top:2px;">Incluye instrucciones, 4 ejemplos (cliente natural, proveedor NIT, cliente+proveedor, Consumidor Final) y hojas de referencia con códigos DIAN y cuentas de tu empresa.</div>
            </div>
            <a href="{{ route('third-parties.import.template') }}"
                    ...
                    <tbody>
                        @foreach ($preview['rows'] as $r)
                            @php
                                ...
                                $roles = [];
                                foreach (['is_customer' => 'CL', 'is_supplier' => 'PR', 'is_employee' => 'EM', 'is_other' => 'OT'] as $k => $abbr) {
                                    $v = $r['data'][$k] ?? '';
                                    if (in_array(strtolower(trim((string) $v)), ['si', 'sí', 'yes', 'true', '1', 'x'], true)) {
                                        $roles[] = $abbr;
                                    }
                                }
                            @endphp
                            <tr style="border-top:1px solid #f1f5f9; background:{{ $bg }};">
                                ...
                                <td style="padding:6px 10px; font-family:ui-monospace, monospace; font-weight:700;">
                                    <span style="color:#6b7280; text-transform:uppercase; font-size:10px;">{{ $r['data']['document_type'] ?? '' }}</span>
                                    {{ $r['data']['document_number'] ?? '—' }}
                                </td>
                                ...
                                <td style="padding:6px 10px;">
                                    @foreach ($roles as $role)
                                        <span style="background:#dcfce7; color:#166534; padding:1px 6px; border-radius:4px; font-size:10px; font-weight:800; margin-right:2px;">{{ $role }}</span>
                                    @endforeach
                                </td>
                                <td style="padding:6px 10px;">
                                    @if ($hasErr)
            ...
            @if ($preview['valid'])
                <div style="margin-top:14px; padding:14px; background:#dcfce7; border:1px solid #16a34a; border-radius:10px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                    ...
                    <button type="button" wire:click="confirmImport"
                        onclick="return confirm('¿Confirmar importación de {{ $s['total'] }} terceros?')"
                ...
                <div style="margin-top:10px; background:#fff; padding:10px 12px; border-radius:8px; max-height:200px; overflow:auto;">
                    ...
                    @foreach ($result['errors'] as $err)
                        <div style="font-size:12px; padding:3px 0; border-top:1px solid #fee2e2;">
                            Fila {{ $err['row_number'] }} — <code>{{ $err['document_number'] }}</code>: {{ $err['message'] }}
                        </div>
                    @endforeach
                </div>
            @endif
