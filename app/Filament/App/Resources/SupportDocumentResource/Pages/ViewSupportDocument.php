...
class ViewSupportDocument extends ViewRecord
{
    protected static string $resource = SupportDocumentResource::class;

    public function getTitle(): string
    {
        return 'Documento Soporte '.$this->record->fullNumber();
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('post')
                ...
                ->color('success')
                ->visible(fn (PurchaseInvoice $record) => $record->status === 'draft'
                    && auth()->user()?->can('support_documents.post'))
                ->requiresConfirmation()
                ->modalHeading('Contabilizar documento soporte')
                ->modalDescription(fn (PurchaseInvoice $record) => sprintf(
                    'Vas a contabilizar el documento soporte %s por $%s. Se generará el asiento contable y los movimientos de inventario. No podrás editarlo después.',
                    $record->fullNumber(),
                ...
                ->action(function (PurchaseInvoice $record) {
                    try {
                        $document = app(PurchaseInvoiceEngine::class)->post($record);
                        Notification::make()
                            ->success()
                            ->title('Documento soporte contabilizado')
                            ->body("Asiento {$document->journalEntry?->fullNumber()} creado.")
                            ->send();
        ...
        return $infolist->schema([
            Infolists\Components\Section::make('Cabecera')
                ...
                ->schema([
                    ...
                    Infolists\Components\TextEntry::make('supplier.document_number')->label('Documento proveedor')->placeholder('—'),
            ...
            Infolists\Components\Section::make('Asiento contable')
                ->visible(fn (PurchaseInvoice $record) => $record->journal_entry_id !== null)
                ->schema([
                    Infolists\Components\TextEntry::make('journalEntry.full_number')
                        ->label('Asiento')
                        ->state(fn (PurchaseInvoice $record) => $record->journalEntry?->fullNumber())
                        ->url(fn (PurchaseInvoice $record) => $record->journalEntry
                            ? route('filament.app.resources.journal-entries.view', $record->journalEntry)
                            : null),
                ]),

            Infolists\Components\Section::make('Notas / concepto')
                ->visible(fn (PurchaseInvoice $record) => $record->notes !== null)
                ->schema([
